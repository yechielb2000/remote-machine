...
class FakeProtocol:
    ...
    def exec(self, command: str, state: RemoteState) -> CommandResult:
        ...
        if "rev-parse --abbrev-ref HEAD" in command:
            return CommandResult(command=command, exit_code=0, stdout="main\n", stderr="")
        ...
        # Mock git log
        if "log" in command and "--format=" in command:
            return CommandResult(
                command=command,
                exit_code=0,
        ...
        # Mock git diff
        if "diff" in command and "--stat" not in command:
            return CommandResult(
                command=command,
                exit_code=0,
        ...
        # Mock git diff --stat
        if "diff --stat" in command:
            return CommandResult(
                command=command,
                exit_code=0,
                stdout="file1.py | 10 2\nfile2.py | 5 1\nfile3.py | 2 1\n 3 files changed, 14 insertions(+), 3 deletions(-)\n",
                stderr="",
            )
